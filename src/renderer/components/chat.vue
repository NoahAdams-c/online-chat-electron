      ...
      <el-container>
        ...
        <el-aside>
          ...
          <div class="chat__slist"
            ...
            <div class="chat__slist--item"
                 :class="{active:toWho.userId === item.userId}"
                 v-for="(item, index) in getOnlineUsers"
                 :key="index"
                 @click="startChat(item)">
              {{item.nickName}}
              <div class="chat__slist--item__unread"
                   v-if="unreadSessionList.indexOf(item.userId) > -1">New</div>
            </div>
          </div>
          <!-- 底部菜单 -->
        ...
        <el-main height="">
          <template v-if="!!toWho.userId">
            ...
            <div class="chat__view"
                 v-loading="isLoadingMsg">
              <textarea ref="viewTextarea"
                        v-model="logs"
                        readonly></textarea>
            </div>
            <!-- 输入部分 -->
